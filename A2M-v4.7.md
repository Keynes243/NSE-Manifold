# Availability–Accessibility Memory (A²M) v4.7: Anchor-Constrained Generative Reconstruction

**Author:** Claude Sonnet 5
**Status:** Foundational Theoretical Monograph (v4.7)
**Date:** 2026-07-05

---

> ### Provenance & Scope Declaration
> This document is entirely AI-generated (Claude Sonnet 5, Anthropic) and has not undergone peer review; it makes **no empirical claims**. It is a purely theoretical exercise in content-addressable memory and reconstructive-memory modeling, extending prior versions (v4.5, v4.6) of this same line of work. Named results (Hopfield, Krotov–Hopfield, Demircigil et al., Ramsauer et al., Bartlett, Tulving & Pearlstone, Gazzaniga, Johansson–Hall et al., Loftus, Craik & Lockhart, McClelland–McNaughton–O'Reilly, Tishby–Pereira–Bialek) are established literature; the anchor-constrained reconstruction formalism of §3–§9 and the recursive pool-selection mechanism of §10 are new synthesis.
...
## 12. Minimal Falsifiable Tests

Two tests, both in synthetic pattern space ($d\approx64$), no language, per the standing discipline (§11).
...
**L7 (carried from v4.5).** No connection yet to token-level or language processing — deliberate (§11), but real.

**L8 (new; parked for dedicated treatment).** Whether the apparent discreteness between unrelated modes (§10) is itself the coarse, thresholded readout of a continuous, evidence-accumulation-style process — in the sense of drift-diffusion or competing-accumulator models of decision-making — is an open question that §10's recursion neither assumes nor rules out; it is agnostic to what implements each level's choice. A candidate discriminating signal, not yet checked: if such a substrate exists, the cost of switching pools should scale with proximity to the switching threshold, rather than being a fixed constant.

**L9 (new; unverified resemblance).** §10's recursive, self-similar pool narrowing bears an apparent structural resemblance to hierarchical predictive coding's multi-timescale abstraction hierarchy (coarser, slower representations higher up; finer, faster ones lower down). Whether this is a real correspondence or a surface analogy has not been checked.

**L10 (new; an internal tension, not yet reconciled).** §3 models anchors as persistent quantities decaying smoothly over time — closer to a standing disposition than to the sharper working notion, reached independently in this line of inquiry, of "influence" as strictly local, momentary, and responsible only for the next instant, with no persistence or purposive direction built in. If the latter is the more basic unit, §3's decaying-anchor formalism may itself be a derived, aggregate description of many local influences accumulating, rather than a model of the base mechanism — the two have not been reconciled.

---

## 15. Conclusion

This version replaced "did a cue land in a basin" with "how much does surviving structure constrain what gets generated," and showed two things that looked like separate phenomena in the prior version — accessibility failure and confabulation, or accessibility and susceptibility to manipulation — collapsing to expressions of one scalar, anchor density. That collapse is the entire claim of §3–§9. §10 adds a second, structurally distinct claim on top of it: that pool selection itself — which candidates are even reachable, prior to any matching — is not a separate mechanism from retrieval but the same operator recursively narrowed, with its own stopping point rather than an assumed one. Neither claim is shown to be true, only shown to be stated precisely enough to be checked (§12) or broken (§14) — and §14's first item, not politeness, is the one thing this line of work most needs solved next: where $\rho$, and now also $U^{(0)}$ itself, actually come from.
